...
function GroupsTab({ projectId, project }: { projectId: string; project: Project }) {
  ...
  const autoCreateGroups = async () => {
    ...
    for (let i = 0; i < groupCount; i++) {
      newGroups.push({ name: groupNames[i], members: [] })
    }

    shuffled.forEach((player, i) => {
      newGroups[i % groupCount].members.push(player)
    })
  ...
  return (
    <>
      ...
      {groups.length === 0 ? (
        <div className="empty">조편성이 없습니다<br />자동 조편성 버튼을 눌러주세요</div>
      ) : (
        <>
          {groups.map(g => (
...
function MatchesTab({ projectId, project }: { projectId: string; project: Project }) {
  ...
  return (
    <>
      {matches.length === 0 ? (
        <div className="empty">경기가 없습니다<br />조편성 탭에서 경기를 생성하세요</div>
      ) : (
        <>
