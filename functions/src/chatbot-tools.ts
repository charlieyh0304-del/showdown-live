  ...
  try {
    switch (name) {
      ...
      case "get_tournament_rankings": {
        ...
        if (!tSnap.exists()) return JSON.stringify({ error: "대회를 찾을 수 없습니다." });
        ...
        if (!mSnap.exists()) return JSON.stringify({ tournamentName: tName, rankings: [], message: "경기 정보 없음" });
        ...
        // 2) 전체 통합 순위 (모든 경기 기반, 프론트엔드 calculateIndividualRanking과 동일)
        ...
        const rankings: Array<{ rank: number; name: string; wins: number; losses: number; sets: string }> = [];
