 ...
 */
import { asString, asNumber, asBoolean } from "../db-helpers";
...
export async function createTeamLeague(input: Record<string, unknown>, executeTool: ExecuteTool): Promise<string> {
  const steps: string[] = [];
  const tlIsFullLeague = asString(input.format) === "full_league" || asNumber(input.groupCount) === 1;
  const tlGroupCount = tlIsFullLeague ? 1 : asNumber(input.groupCount, 2);
  ...
  const tlResult = await executeTool("setup_full_tournament", {
    ...
    type: asBoolean(input.randomTeam) ? "randomTeamLeague" : "team",
    teams: input.teams,
    groupCount: tlGroupCount,
    advancePerGroup: tlIsFullLeague ? 0 : asNumber(input.advancePerGroup, 2),
    qualifyingWinScore: asNumber(input.qualifyingWinScore, 31),
  ...
  const tlTid = asString(tlParsed.tournamentId);
  ...
  // 4. 스케줄 생성
  const matchDur = asNumber(input.matchDurationMinutes, 60);
  const teamRest = asNumber(input.teamRestMinutes, 30);
  const schedResult = await executeTool("generate_schedule", {
    tournamentId: tlTid, scheduleDate: asString(input.date),
