...
import PracticeCoinToss from '../../components/practice/PracticeCoinToss';
import PracticeScoringPanel from '../../components/practice/PracticeScoringPanel';
...
export default function PracticeScoring() {
  ...
  const handleServeMiss = useCallback(() => {
    ...
    handleIBSAScore(servingPlayer as 1 | 2, 'serve_miss', 1, true, `${sName} ${t('common.scoreActions.serveMiss')}`);
  ...
  if (match.status === 'pending') {
    return (
      <PracticeCoinToss
        ...
        pendingFirstServe={pendingFirstServe}
        onTossWinner={setTossWinner}
        onCoinTossStep={setCoinTossStep}
        onPendingFirstServe={setPendingFirstServe}
        onCourtChange={setCourtChangeByLoser}
        onStartMatch={handleStartPracticeMatch}
        onBack={() => navigate('/referee/practice/setup')}
        initAudio={initAudio}
      />
    );
  }
    ...
    <div className="min-h-screen flex flex-col">
      ...
      {showSideChange && sideChangeConfirmed && (
        <TimerModal
          ...
          closeLabel={`🎾 ${t('common.matchHistory.resumeMatch')}`}
          required
          warningText={timerWarningText}
        />
      )}
      ...
      <div className="flex border-b border-gray-700" aria-live="polite">
        <div className="flex-1 flex flex-col items-center py-3 px-2 border-r border-gray-700" style={match.currentServe === 'player1' ? { borderLeft: '3px solid rgba(234,179,8,0.4)' } : undefined}>
          <h2 className="text-lg font-bold text-yellow-400">
            {match.currentServe === 'player1' && <span aria-hidden="true">🎾 </span>}{p1Name}
          </h2>
        ...
        <div className="flex-1 flex flex-col items-center py-3 px-2" style={match.currentServe === 'player2' ? { borderRight: '3px solid rgba(6,182,212,0.4)' } : undefined}>
          <h2 className="text-lg font-bold text-cyan-400">
            {match.currentServe === 'player2' && <span aria-hidden="true">🎾 </span>}{p2Name}
          </h2>
      ...
      <PracticeScoringPanel
        ...
        onIBSAScore={handleIBSAScore}
        onQuickFoul={handleQuickFoul}
        onDeadBall={handleDeadBall}
        onServeMiss={handleServeMiss}
        onUndo={handleUndo}
        onTimeout={handleTimeout}
        onPenalty={handlePenalty}
        onOpenSubModal={(team) => { setSubTeam(team); setSubOutIdx(null); setSubInIdx(null); setShowSubModal(true); }}
        shortWhistle={shortWhistle}
        goalWhistle={goalWhistle}
